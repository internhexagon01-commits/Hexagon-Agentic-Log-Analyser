...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_chat(self):
        ...
        try:
            ...
            status_trail = result.get("status_trail", [])
            print(f"[UI_SERVER] answer length={len(answer)} trail={status_trail}")

            # Prepend status trail as a "thinking" block if available
            if status_trail:
                trail_text = " → ".join(status_trail)
                answer = f"🔍 *{trail_text}*\n\n{answer}"
